<?php
/**
 * Company contacts section for the services archive.
 *
 * @package a4-remont
 */

$fallback_partial = ! empty( $args['fallback_partial'] ) ? (string) $args['fallback_partial'] : 'section/services/company-contacts-2.html';
$section_class    = ! empty( $args['section_class'] ) ? trim( (string) $args['section_class'] ) : '';
$wide_cols        = ! empty( $args['wide_cols'] );
...
if ( '' === trim( $title ) && '' === trim( $text ) && '' === trim( $phone ) && '' === trim( $email ) && '' === trim( $address ) ) {
	a4_remont_render_static_markup( $fallback_partial );
	return;
}
...
$classes      = trim( 'company-contacts ' . $section_class );
$cols_class   = 'company-contacts__cols' . ( $wide_cols ? ' company-contacts__cols--wide' : '' );
?>
<section class="<?php echo esc_attr( $classes ); ?>"<?php echo $section_id ? ' id="' . esc_attr( $section_id ) . '"' : ''; ?>>
	<div class="company-contacts__container _container">
		<div class="company-contacts__head"></div>
		...
		<div class="<?php echo esc_attr( $cols_class ); ?>">
			...
			<div class="company-contacts__col company-contacts__col--right">
				<?php if ( $text ) : ?>
					<p class="company-contacts__text"><?php echo nl2br( esc_html( $text ) ); ?></p>
				<?php endif; ?>
			</div>
		</div>
